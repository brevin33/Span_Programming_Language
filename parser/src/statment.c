...
Assignment createAssignmentFromTokens(Token** tokens, Function* function, Scope* scope, Project* project) {
    ...
    Expresstion value = createExpresstionFromTokens(&token, tt_endl, function, scope, project);
    ...
    bool staticCast = canStaticCast(value.tid, var->type, project);
    if (!staticCast) {
        ...
        case tt_id: {
            ...
            if (type != 0) {
                if (token->type == tt_dot) {
                    ...
                    token = startToken;
                    Expresstion expresstion = createExpresstionFromTokens(tokens, tt_endl, function, scope, project);
                ...
                token = startToken;
                Declaration declaration = createDeclarationFromTokens(&token, function, scope, project);
