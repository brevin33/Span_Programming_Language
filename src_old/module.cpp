...
optional<Value> Module::parseValue(Scope& scope) {
    ...
    switch (tokens.getToken().type) {
        case tt_id: {
            ...
            // func call
            if (tokens.getToken().type == tt_lpar) {
                optional<Value> val = parseFunctionCall(name, scope, nullptr, {});
                if (!val.has_value()) {
                    tokens.pos = start;
                    return nullopt;
                }
...
optional<Value> Module::parseFunctionCall(string& name, Scope& scope, Value* caller, const vector<Type>& templateTypes) {
